...
public class CrossedWires {
    ...
    private void calculateStepsInput1() {
        Map<String, Point> path1 = walkPath(input1_Wire1);
        Map<String, Point> path2 = walkPath(input1_Wire2);
        List<Point> path1Matches = matches(path1, path2);
        List<Point> path22Matches = matches(path2, path1);
        int shortestMatch = 0;
        for (Point pointPath1 :   path1Matches) {
            for (Point pointPath2:   path22Matches) {
                if(pointPath1.x == pointPath2.x && pointPath1.y == pointPath2.y){
                    if(shortestMatch ==0){
                        shortestMatch = pointPath1.stepsSinceStarts+pointPath2.stepsSinceStarts;
                    }else{
                        if(shortestMatch>pointPath1.stepsSinceStarts+pointPath2.stepsSinceStarts){
                            shortestMatch = pointPath1.stepsSinceStarts+pointPath2.stepsSinceStarts;
                        }
                    }
                }
        ...
        System.out.println("Steps needed "+shortestMatch);
    }

    private void calculateStepsMyInput() {
        Map<String, Point> path1 = walkPath(myInput_Wire1);
        Map<String, Point> path2 = walkPath(myInput_Wire2);
        List<Point> path1Matches = matches(path1, path2);
        List<Point> path22Matches = matches(path2, path1);
        int shortestMatch = 0;
        for (Point pointPath1 :   path1Matches) {
    ...
    public void startRiddle2() {
        this.calculateStepsInput1();
        this.calculateStepsInput2();
        this.calculateStepsMyInput();
    }

    private void calculateStepsInput2() {
        Map<String, Point> path1 = walkPath(input2_Wire1);
        Map<String, Point> path2 = walkPath(input2_Wire2);
        List<Point> path1Matches = matches(path1, path2);
        List<Point> path22Matches = matches(path2, path1);
        int shortestMatch = 0;
        for (Point pointPath1 :   path1Matches) {
    ...
    public void startRiddle1() {
        this.validateInput1();
        this.validateInput2();
        this.validateMyInput();
    }

    private void validateMyInput() {
        Map<String, Point> path1 = walkPath(myInput_Wire1);
        Map<String, Point> path2 = walkPath(myInput_Wire2);
        Point nearest = findNearest(path1, path2);
        System.out.println("Distance myInput: " + nearest.getDistance());
    }
    ...
    private void validateInput2() {
        ...
        Map<String, Point> path2 = walkPath(input2_Wire2);
        Point nearest = findNearest(path1, path2);
        System.out.println("Distance test2: " + nearest.getDistance());
    ...
    private List<Point> matches(Map<String, Point> path1, Map<String, Point> path2) {
        ...
        for (String keyPath1 : path1.keySet()) {
            if (path2.containsKey(keyPath1)) {
                //CrossedPath
                crossedPoints.add(path1.get(keyPath1));
            }
        }
        return crossedPoints;
    }

    private Point findNearest(Map<String, Point> path1, Map<String, Point> path2) {
        List<Point> crossedPoints = matches(path1, path2);

        //  Map<String, Point> doubleDistance = new HashMap<>();
        Point lowest = null;
        int minDist = 999999999;
        for (Point crossedPoint : crossedPoints) {
            // System.out.println(crossedPoint.toString());
            if (crossedPoint.getDistance() <= minDist) {
                minDist = crossedPoint.getDistance();
                lowest = crossedPoint;
            }
        }
        return lowest;
    ...
    private void validateInput1() {
        ...
        Map<String, Point> path2 = walkPath(input1_Wire2);
        Point nearest = findNearest(path1, path2);
        System.out.println("Distance test1: " + nearest.getDistance());
    }
    ...
    private Map<String, Point> walkPath(String pPath) {
        ...
        String[] modifiedPath = pPath.split(separator);
        int stepsTaken = 1;
        for (String direction : modifiedPath) {
            Point nextLocation = null;
            int step = Integer.parseInt(direction.substring(1));
            for (int singleStep = 1; singleStep <= step; singleStep++) {

                switch (direction.charAt(0)) {
                    case 'L':
                        assert whereWeAre != null;
                        nextLocation = new Point(whereWeAre.x - 1, whereWeAre.y, direction, stepsTaken);
                        walkedPath.put(nextLocation.toString(), nextLocation);
                        break;
                    case 'R':
                        assert whereWeAre != null;
                        nextLocation = new Point(whereWeAre.x + 1, whereWeAre.y, direction, stepsTaken);
                    ...
                    case 'U':
                        assert whereWeAre != null;
                        nextLocation = new Point(whereWeAre.x, whereWeAre.y + 1, direction, stepsTaken);
                        walkedPath.put(nextLocation.toString(), nextLocation);
                        break;
                    case 'D':
                        assert whereWeAre != null;
                ...
                //   buffer.append(nextLocation.x).append(separator).append(nextLocation.y).append("\n");
                whereWeAre = nextLocation;
                stepsTaken++;
            }
        }
        return walkedPath;
    ...
    static class Point {
        ...
        Point(int pX, int pY, String pLastDirection, int pStepsSinceStarts) {
            x = pX;
            y = pY;
            lastDirection = pLastDirection;
            stepsSinceStarts = pStepsSinceStarts;
        }

        int getDistance() {
            int neutralX = x < 0 ? x * -1 : x;
            int neutralY = y < 0 ? y * -1 : y;
            return (neutralX + neutralY);
        }
    }
}
